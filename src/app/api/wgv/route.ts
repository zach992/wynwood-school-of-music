import { NextRequest } from "next/server";
import { airtableCreate } from "@/lib/airtable";
import { sendFormNotification } from "@/lib/email";
import { buildWgvEmail } from "@/lib/email-templates";
import { asArray, calcAge, checkSpamGuard, joinNonEmpty } from "@/lib/form-utils";

export async function POST(req: NextRequest) {
  let body: Record<string, unknown>;
  try {
    body = await req.json();
  } catch {
    return new Response(null, { status: 400 });
  }

  if (checkSpamGuard(body)) return new Response(null, { status: 200 });

  const { website: _hp, _renderedAt: _t, ...p } = body;
  const studentName = joinNonEmpty(p.firstName, p.lastName);
  const instruments = asArray(p.instruments);

  const tableName = process.env.AIRTABLE_WGV_TABLE || "Walt Grace Lesson Signups";

  try {
    await airtableCreate(tableName, {
      "Student Name": studentName || "(no name)",
      Submitted: new Date().toISOString(),
      Email: p.email,
      Phone: p.phone,
      "Student DOB": p.dob,
      "Student Age": calcAge(p.dob),
      Instruments: instruments,
      "Experience Level": p.experience,
      "Additional Info": p.notes,
      "Lead Status": "New",
    });
  } catch (err) {
    console.error("[api/wgv] Airtable write failed:", err);
    return new Response(JSON.stringify({ error: "Save failed" }), {
      status: 502,
      headers: { "content-type": "application/json" },
    });
  }

  if (process.env.RESEND_API_KEY) {
    sendFormNotification(buildWgvEmail(p, calcAge(p.dob))).catch((err) =>
      console.error("[api/wgv] Resend email failed:", err)
    );
  }

  const webhookUrl = process.env.ZAPIER_WGV_WEBHOOK_URL;
  if (webhookUrl) {
    fetch(webhookUrl, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ...p, _form: "wgv", _submittedAt: new Date().toISOString() }),
    }).catch((err) => console.error("[api/wgv] Zapier forward failed:", err));
  }

  return new Response(null, { status: 200 });
}
