...
export async function POST(req: NextRequest) {
  ...
  const sessions = asArray(p.sessions);
  const genres = asArray(p.genres);
  ...
  try {
    await airtableCreate(tableName, {
      ...
      "Primary Instrument": p.instrument,
      "Experience Level": p.experience,
      Sessions: sessions.length ? sessions.map((s) => `• ${s}`).join("\n") : "",
      Genres: genres,
  ...
  if (process.env.MAILCHIMP_API_KEY && typeof p.parentEmail === "string" && p.parentEmail) {
    ...
    const tags = ["Lead — Summer Camp", `Website Lead ${year}`];
